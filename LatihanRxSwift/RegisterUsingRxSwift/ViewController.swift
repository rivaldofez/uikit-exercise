...
class ViewController: UIViewController {
    ...
    private func setupRx(){
        let nameStream = nameTextField.rx.text
            .orEmpty
            .skip(1)
            .map{ !$0.isEmpty} //to make sure value not empty
        ...
        let confirmationPasswordStream = Observable.merge(
            confirmPasswordTextField.rx.text.orEmpty.skip(1).map{
                $0.elementsEqual(self.passwordTextField.text ?? "")
            },
            
            passwordTextField.rx.text.orEmpty.skip(1).map{
                $0.elementsEqual(self.confirmPasswordTextField.text ?? "")
            }
        )
        
        confirmationPasswordStream.subscribe(onNext: { value in
